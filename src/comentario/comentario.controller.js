...
export const createComentario = async (req, res) => {
  try {
    const { comentario, nombre, publicacion, parentComentario } = req.body;

    if (!comentario || !nombre || !publicacion) {
      return res.status(400).json({
        success: false,
        msg: "Todos los campos (comentario, nombre, publicacion) son requeridos",
      });
    }

    if (!mongoose.Types.ObjectId.isValid(publicacion)) {
      return res.status(400).json({
        success: false,
    ...
    const nuevoComentario = new Comentario({
      ...
      nombre,
      publicacion,
      parentComentario: parentComentario || null // ✅ Guardamos si viene
    });
